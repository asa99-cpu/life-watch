...
# Small Watch at the Top
def draw_small_watch(current_age, lifespan):
    # Calculate percentage of life lived
    life_percentage = current_age / lifespan

    # Adjust theta to start from zero at the bottom of the clock
    theta = np.linspace(0, 2 * np.pi, 1000) + np.pi / 2

    # Create the figure
    fig, ax = plt.subplots(figsize=(3, 3))  # Smaller size for the header
    ax.axis("equal")
    ...
    ax.plot([0, np.cos(angle)], [0, np.sin(angle)], color="black", linewidth=2)
...
def draw_real_watch(current_age, lifespan):
    ...
    # Create the figure
    fig, ax = plt.subplots(figsize=(6, 6))  # Larger size for the main watch
    ax.axis("equal")
...
def main():
    ...
    st.markdown(
        """
        <div style="background-color:#f0f0f5; border: 1px solid #ccc; padding: 10px; text-align:center;">
            <h1 style="font-size: 28px; margin: 0;">Life Watch</h1>
        </div>
        """,
        unsafe_allow_html=True,
    )

    # Add the small watch at the top
    st.pyplot(draw_small_watch(current_age, lifespan))
